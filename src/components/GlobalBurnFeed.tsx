...
interface GlobalBurnFeedProps {
  ...
  personalBurnHistory: BurnTransaction[];
}
...
export default function GlobalBurnFeed({ personalBurnHistory = [] }: GlobalBurnFeedProps) {
  ...
  const labels = {
    ...
    personalBurns: "TRANSACTIONS",
    noHistory: "No transactions recorded yet. Use BurnerSol to burn your accounts.",
    loading: "Loading...",
    globalFeedTitle: "GLOBAL BURN FEED",
    globalFeedSubtitle: "Real-time verified burn transactions",
    globalFeedColAddress: "Address",
    globalFeedColAmount: "Items",
    globalFeedColAsset: "Asset",
    globalFeedColSolReclaimed: "SOL Reclaimed",
    globalFeedTxLabel: "NEW",
  };
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 font-mono text-[9px] text-slate-500 uppercase tracking-widest bg-black/20">
                <th className="p-4 pl-6">{labels.globalFeedColAddress}</th>
                <th className="p-4">{labels.globalFeedColAmount}</th>
                <th className="p-4">{labels.globalFeedColAsset}</th>
                <th className="p-4 text-right pr-6">{labels.globalFeedColSolReclaimed}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.04]">
              {isLoading ? (
                ...
                filteredFeed.map((item, idx) => {
                  ...
                  const isNew = idx === 0 && true && item.time === "Just now";
                  return (
                          ...
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              ...
                              <span className="text-[8px] text-slate-500 font-mono tracking-wider uppercase block">
                                {isNew ? (
                                  <span className="text-flame-orange font-bold animate-pulse">● {labels.globalFeedTxLabel}</span>
                                ) : (
                                  item.time
                                )}
                                {item.walletName && (
